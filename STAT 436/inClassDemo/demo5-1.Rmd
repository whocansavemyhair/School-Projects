...
fires <- read.csv("https://uwmadison.box.com/shared/static/k5vvekf1bhh9e16qb9s66owygc70t7dm.csv") %>%
...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(4, plotOutput("histogram", brush = brushOpts("plot_brush", direction = "x"))), ## update to make this brushable.
  ),
...
server <- function(input, output) {
  ...
  observeEvent(
    input$plot_brush,{
      ...
      new_values <- brushedPoints(fires, input$plot_brush, allRows = TRUE)$selected_
