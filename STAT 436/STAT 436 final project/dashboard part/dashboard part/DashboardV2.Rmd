...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem("Department Trends", tabName = "dashboard", icon = icon("chart-line"))
    )
  ),
    ...
    tabItems(
      ...
      tabItem(tabName = "widgets",
              tabItem(tabName = "slider2",
                      ...
                      fluidRow(
                        box(title = "OVERVIEW DEPARTMENTS : WHAT ARE THE DIFFERENT DEPARTMENTS?? ANY GRAPHICAL WAY TO SHOWCASE THEM??", status = "primary", solidHeader = TRUE, width = 12, height = 600)
                      ),
                      
                      fluidRow(
                        box(title = "TOTAL EXPENDITURE PER YEAR (2008 - 22) | Lineplot or Barplot", status = "primary", solidHeader = TRUE, width = 12, height = 400)
                      ),
                      ...
                      fluidRow(
                        box(title = "", width = 12, align = "center", height = 500, solidHeader = TRUE,
                            box("Top 5 Departments", width = 3, solidHeader = TRUE, background = "black"), 
                            box("PIE CHART OF DEPT EXPENDITURE FOR SELECTED YEAR??", width = 6, status = "warning", solidHeader = TRUE), 
                            box("Bottom 5 Departments", width = 3, solidHeader = TRUE, background = "black"))
                        
                      ),
                      
                      fluidRow(
                        box(title = "Treemap? Based on year selection from above?", width = 12, align = "center", height = 500, status = "primary", solidHeader = TRUE)
                      ),
      ...
      tabItem(tabName = "dashboard",
              fluidRow(
                column(9, offset = 4, align = "center",
                       box(title = "DEPARTMENT BREAKDOWNS", status = "primary", 
                           selectInput("departmentVal", "Choose a department:", choices = unique(annual$name)))
                )
              ),
              fluidRow(
                box(title = "Expenditure Breakdown for Selected Department", status = "primary", solidHeader = TRUE, width = 12,
                    plotlyOutput("plot1", height = 600))
              ),
              fluidRow(
                column(9, offset = 4, align = "center",
                       box(title = "DEPARTMENT-WISE COMPARISONS", status = "primary",
                           selectizeInput("tab3_departments", "Choose departments to compare:", code_departments$name, 
                                          selected = c("University of Wisconsin System", "Administration, Department of"),
                                          multiple = TRUE
                           )))),
              fluidRow(
                box(title = "Comparison of Expenditure over Years for Selected Departments", status = "primary", solidHeader = TRUE, width = 12,
                    plotlyOutput("plot2"))
              ),
              fluidRow(
                box(title = "Comparison of Percentage of Yearly Budget for Selected Departments", status = "primary", solidHeader = TRUE, width = 12,
                    plotlyOutput("plot3"))
              ),
              fluidRow(
                column(9, offset = 4, align = "center",
                       box(title = "Granular - View of Expenditure Breakdown for Departments", status = "primary",
                           sliderInput(inputId = "pie_bar_year",
                                       ...
                                       value=2022,
                                       sep=""),
                           selectizeInput("pie_bar_department", "Select a Department:", selected ="University of Wisconsin System", choices = code_departments$name)))
              ),
...
server <- function(input, output) {
  ...
  observeEvent(
    input$pie_bar_department,
    dept_reactive(input$pie_bar_department))
