...
category_expend_bar_chart <- function(df, title){
  # df <- df %>% 
  #   mutate(name = fct_reorder(name, desc(total_expenditure)))
  ...
  plot_ly(df,
    ...
    layout(title = title,
           ...
           yaxis = list(title = "Expenditure (dollors)"))
}
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "Tab1",
              ...
              fluidRow(
                box(title = "INTRODUCTION", status = "primary", solidHeader = TRUE, width = 12, height = 750,
                    box(title =h1("Introduction", style = 'font-size:42px;color:black;') , status = "primary", solidHeader = TRUE,
                        "
                  ...
                  ", style = 'font-size:20px;color:black;'), 
                    box("",status = "primary", solidHeader = TRUE,
                        img(src='https://uwmadison.box.com/shared/static/7777ho5eikqjcyra0xloshs17m5h3qtz', align = "center")))
              ),
              fluidRow(
      ...
      tabItem(tabName = "Tab2",
              ...
              fluidRow(box(title = "Treemap-Overview")),
              fluidRow(plotOutput("treemap")),
              
              fluidRow(box(title = "Expenditure by depart. & category")),
              
              fluidRow(align = "center", height = 500, 
                       #PIE CHART & BARPLOT ???
                       column(width=6, plotlyOutput("tab2_piechart")),
                       column(width=6, plotlyOutput("tab2_barchart")))
      ),
...
server <- function(input, output) {
  ...
  output$tab2_barchart <- renderPlotly({
    ...
    category_expend_bar_chart(df, dept_reactive())
  })
  ...
  output$treemap <- renderPlot({
    ggplot(tab2_dept_subset(), aes(area = amount, fill = name, subgroup = department_name)) +
      geom_treemap() +
