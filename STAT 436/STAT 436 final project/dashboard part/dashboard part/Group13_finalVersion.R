...
annual <-
  load_data(
    './annual.csv',
    'https://uwmadison.box.com/shared/static/6w1pooo9mkh9dkg2yz2p5sffa2j01y48.csv'
  )
depat <-
  load_data(
    './bydept.csv',
    'https://uwmadison.box.com/shared/static/tvs8zrvm9casu8eujdk5z4dglrni8e5o.csv'
  )
...
department_proportion_pie_chart <-
  function(df, labels, values = "amount", source) {
    plot_ly(
      df,
      labels = df[[labels]],
      values = df[[values]],
...
ui <- dashboardPage(
  ...
  dashboardSidebar(width = 300,
                   sidebarMenu(
                     ...
                     menuItem(
                       "Expenditure by Depart. & Category",
                       tabName = "Tab2",
                       icon = icon("list-alt")
                     ),
  ...
  dashboardBody(tabItems(
    ...
    tabItem(
      ...
      fluidRow(
        box(
          title = "TOTAL EXPENDITURE PER YEAR (2008 - 22) | Lineplot",
          status = "primary",
          solidHeader = TRUE,
          width = 12,
          height = 500,
      ...
      fluidRow(column(
        ...
        align = "center",
        box(
          title = "YEARLY DEPARTMENT EXPENDITURE SNAPSHOT",
          status = "primary",
          selectInput(
            inputId = "yearOverview",
            label = "Select a Year",
            choices = 2008:2022,
            selected = 2022
          )
      ...
      fluidRow(
        box(
          ...
          box(
            ...
            width = 3,
            solidHeader = TRUE,
            dataTableOutput("tab1_table1")
          ),
          box(
            "PIE CHART OF EXPENDITURE FOR SELECTED YEAR",
            width = 6,
            status = "warning",
            solidHeader = TRUE,
    ...
    tabItem(
      ...
      fluidRow(column(
        ...
        align = "center",
        box(
          title = "Granular - View of Expenditure Breakdown for Departments",
          status = "primary",
          sliderInput(
            inputId = "pie_bar_year",
            label = "Year",
            min = 2008,
            max = 2022,
            value = c(2022, 2022),
    ...
    tabItem(
      ...
      fluidRow(
        box(
          title = "Expenditure Breakdown for Selected Department",
          status = "primary",
          solidHeader = TRUE,
          width = 12,
      ...
      fluidRow(column(
        ...
        align = "center",
        box(
          title = "DEPARTMENT-WISE COMPARISONS",
          status = "primary",
          selectizeInput(
            "tab3_departments",
      ...
      fluidRow(
        box(
          title = "Comparison of Expenditure over Years for Selected Departments",
          status = "primary",
          solidHeader = TRUE,
          width = 12,
          plotlyOutput("plot2")
      ...
      fluidRow(
        box(
          title = "Comparison of Percentage of Yearly Budget for Selected Departments",
          status = "primary",
          solidHeader = TRUE,
          width = 12,
          plotlyOutput("plot3")
...
server <- function(input, output) {
  ...
  dept_subset <- reactive({
    depat %>% filter(year <= input$pie_bar_year[2] &
                       year >= input$pie_bar_year[1] & !is.na(code))
  })
  ...
  output$tab1_piechart <- renderPlotly({
    tab1_annual_subset() %>%
      ...
      department_proportion_pie_chart(labels = "Name",
                                      values = "Amount",
                                      source = "pie")
  })
  ...
  output$tab2_piechart <- renderPlotly({
    annual %>%
      filter(year <= input$pie_bar_year[2] &
               year >= input$pie_bar_year[1] & !is.na(code)) %>%
      arrange(desc(amount)) %>%
      mutate(percentage = amount / sum(amount) * 100) %>%
  ...
  output$plot1 <- renderPlotly({
    ...
    depat %>%
      filter(department_name == input$departmentVal &
               !is.na(code)) %>%
  ...
  output$plot2 <- renderPlotly({
    comparison_plot(
      ...
      "amount",
      "Expenditure ($)",
      scales::number_format(scale = 1e-6, suffix = "M")
    )
  })
  
  output$plot3 <- renderPlotly({
    comparison_plot(tab3_annual_subset(),
                    "percentage",
                    "% of year budget",
                    scales::percent)
  })
