...
_[Reading](https://juliasilge.com/blog/cocktail-recipes-umap/), [Recording](https://mediaspace.wisc.edu/media/Week%2010%20%5B2%5D%20Principal%20Components%20Analysis%20I/1_xc5q0v9t), [Rmarkdown](https://github.com/krisrs1128/stat479/blob/master/_posts/2021-03-24-week10-2/week10-2.Rmd)_
...
Comments: Recipe is a funcition which can specify a series of data porcessing steps. After update_role set `name` and `category` as "id", step_normalized and step_pca will normalize the data and apply pca to the data. Then the prep function start the steps defined by recipe function. And bake funcion can apply those processing steps to new data.
...
Summary: PCA is a linear transformation based on SVD decomposition, which can find a set of eigenvectors that maximize the variance when the data is projected onto them. These eigenvalues are also called principle components. However, unlike the raw data, it's difficult to interpret the columns after transformation.